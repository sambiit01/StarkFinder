...
2. Set your DATABASE_URL environment variable:
   - `set DATABASE_URL=postgres://postgres:postgres@localhost:5432/starkfinder_dev` (Windows)
   - Use your own user/password if different
...
4. Prepare offline SQLx metadata:
   - `cargo sqlx prepare`

5. Build and check code:
   - `cargo check`
   - Run main to confirm database connectivity and health.

Notes:
- The PgPool factory and health_check are in `src/db.rs`.
- SQLx is used with features: `runtime-tokio, postgres, macros`.
- Migrations folder is ready for changes.
